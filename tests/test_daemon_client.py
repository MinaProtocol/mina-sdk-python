...
def test_json_decode_error():
    ...
    client = MinaDaemonClient(
        graphql_uri=GRAPHQL_URL, retries=1, retry_delay=0.0, timeout=5.0
    )
...
def test_http_500_retried():
    ...
    client = MinaDaemonClient(
        graphql_uri=GRAPHQL_URL, retries=2, retry_delay=0.0, timeout=5.0
    )
    with pytest.raises(ConnectionError, match="after 2 attempts"):
        client.get_sync_status()
    client.close()
...
def test_daemon_connection_error_alias():
    ...
    respx.post(GRAPHQL_URL).mock(side_effect=httpx.ConnectError("refused"))
    client = MinaDaemonClient(
        graphql_uri=GRAPHQL_URL, retries=1, retry_delay=0.0, timeout=1.0
    )
